    ...
    if (file) {
      setFormData((prev) => {
        ...
        const docIndex = existingDocuments.findIndex(
          (doc) => doc.igc_document_id === documentTypesSetupId
        );
        ...
        if (docIndex !== -1) {
          ...
          updatedDocuments[docIndex].files = [file]; // Ensuring only the first file
        } else {
          // Add new entry
          updatedDocuments = [
            ...existingDocuments,
        ...
        return {
          ...prev,
          [fieldName]: file, // Keeping a direct reference if needed
        ...
        formData[key].forEach((doc, index) => {
          if (doc.files.length > 0) {
            formDataToSend.append(
              `IGC_Supporting_Documents[${index}].files[0]`,
              doc.files[0]
            );
          }
          formDataToSend.append(
